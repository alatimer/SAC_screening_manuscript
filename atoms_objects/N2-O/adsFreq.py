...
pw = 476
dw = 4760
psppath = '/home/users/alatimer/bin/psp/gbrv/'
dipole = {'status':True}
spinpol = True
...
vibrateatoms=[17,18,19,20]	 # calculate the vibration modes of atoms 

cwd = os.getcwd()
mtl = cwd.split('/')[-1].split('-')[0]

atoms = read('%s-Gr-N2O-dis-TS.cif'%mtl)

if spinpol == True:
    for atom in atoms:
        if atom.symbol == 'O':
            atom.magmom =  3
        if atom.symbol in ['Au','Co','Ge','Pd','Rh','Ru']:
            atom.magmom =  3
